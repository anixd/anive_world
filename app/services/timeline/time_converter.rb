...
class Timeline::TimeConverter
  ...
  # Напр: from_absolute(absolute_year: 7034, to_calendar_id: 2) -> "1712 год Откровения"
  def from_absolute(absolute_year:, to_calendar_id:)
    calendar = @calendars[to_calendar_id]
    return "N/A" unless calendar && absolute_year

    # Высчитываем год относительно "нулевой точки" календаря.
  ...
  # Конвертируем из даты в конкретном календаре в абсолютный год
  ...
  def to_absolute(year:, from_calendar_id:)
    ...
    # Год "до нашей эры" просто прибавляется (он уже отрицательный)
    if year <= 0
  ...
  # Aux метод для красивого форматирования
